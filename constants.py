...
def init() -> None:
    import pygame
    global WIDTH, HEIGHT, CELL_SIZE, BOARD_SZ, DOMINO_NUM,\
    ...
    print(f"[constants] Desktop resolution detected: {WIDTH}×{HEIGHT}")
    #WIDTH /= 2
    #HEIGHT /= 2
    # Grid / display ----------------------------------------------------
    BOARD_SZ = 16  # squares along each board edge
    CELL_SIZE = int(min(HEIGHT, WIDTH)/16)  # px per square
    DOMINO_NUM = 14  # loose dominoes in sidebar
    SIDEBAR_WIDTH = WIDTH - BOARD_SZ * CELL_SIZE
    SIDEBAR_PAD = (HEIGHT - (DOMINO_NUM * CELL_SIZE)) /(DOMINO_NUM + 3)
...
# Optional: make the public surface explicit
__all__ = [
